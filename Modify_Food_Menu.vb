    ...
    Private Sub searchtxt_TextChanged(sender As Object, e As EventArgs) Handles searchtxt.TextChanged
        If searchtxt.Text <> "" Then
            Try
                conOpen()
                cmd.Connection = conn
                cmd.CommandText = "SELECT code, image, name, price, category, quantity, inventory FROM foodmenu WHERE name LIKE '%" & searchtxt.Text.Trim & "%' order by code asc"
                da.SelectCommand = cmd
                dt.Clear()
                da.Fill(dt)
                dgv1.DataSource = dt
                ...
                conn.Close()
            Catch ex As Exception
                MessageBox.Show(ex.Message, "Error | Searching Data")
                conn.Close()
            End Try
        Else
            loadData()
